...
*/
    .AddAttribute ("MaxTimeout",
                   ...
                   TimeValue(MicroSeconds(15000.0)),
                   MakeTimeAccessor(&SwitchedEthernetNetDevice::m_maxTimeout),
                   MakeTimeChecker())
    .AddAttribute ("MaxAverageDelay",
  		   "The maximum average delay that the algorithm can accept and it modifies Tc accordingly",
		   DoubleValue (1.0),
		   MakeDoubleAccessor (&SwitchedEthernetNetDevice::m_maxAverageDelay),
...
//			NS_LOG_UNCOND ("Schedule TransmitCompleteEvent in " << tEvent.GetSeconds () << "sec");
...
SwitchedEthernetNetDevice::TransmitCompleteEvent (void)
{
  ...
  //Remove NS_LOG_UNCOND ("Schedule TransmitReadyEvent in " << m_tInterframeGap << "sec. Now: " << Simulator::Now());  // Remove
...
{
	  //Remove    NS_LOG_UNCOND("SwitchedEthernetNetDevice::CancelGap() Cancelling GAP state in case queue is empty and channel is inactive. " << Simulator::Now());   // Remove
  //m_txMachineState = READY;
...
SwitchedEthernetNetDevice::GetTxMachineState (void)
  ...
  void
SwitchedEthernetNetDevice::PrintMessage(void)
  {
//	  NS_LOG_UNCOND("SwitchedEthernetNetDevice::PrintMessage Message OK! m_deviceId: " << m_deviceId << ", m_otherEndLink: " << m_otherEndLink);
  }
...
SwitchedEthernetNetDevice::TransmitReadyEvent (void)
{
  ...
  //
  //REMOVE cout << Simulator::Now() << " " <<m_txMachineState << " " << m_queue->GetNPackets() << " " << m_otherEndLink->GetQueue()->GetNPackets() << " " << ChannelIsActive() << endl;
...
//		m_txMachineState = READY;
		...
		//Remove 	 NS_LOG_UNCOND("SwitchedEthernetNetDevice::TransmitReadyEvent()1 deviceId: " << m_deviceId << ", queue is empty and channel is inactive. Assign id_sleep: " << Simulator::Now());   // Remove
	  EnableSleep();
    ...
    {
	  //Remove    NS_LOG_UNCOND("SwitchedEthernetNetDevice::TransmitReadyEvent()2 deviceId: " << m_deviceId << ", queue is empty but channel is active. Just wait: " << Simulator::Now());   // Remove
	  m_txMachineState = READY;
	  return;
...
/*  else if(!m_otherEndLink->GetQueue()->IsEmpty() || ChannelIsActive())
  {
	  NS_LOG_UNCOND("SwitchedEthernetNetDevice::TransmitReadyEvent() queue is empty but channel is active or the other queue is not empty. Just wait: " << Simulator::Now());
	  m_txMachineState = READY;
  }
...
SwitchedEthernetNetDevice::Sleep(void)
  {
	  if (Simulator::Now() > simulation_end)
	  {
        ...
        cout << "Now: " << Simulator::Now() << " deviceID: " << m_deviceId << " total: " << simulation_total.GetSeconds() << " LPI: " << total_LPI.GetSeconds() <<" PowerSave: "<< total_LPI.GetSeconds()/simulation_total.GetSeconds()*100 << " %, Average Queue: " << meanQueue/n << " pkts, Average TimeOut: " << meanTimer/n << " s, " << endl;
	  }
//	  NS_LOG_UNCOND("SwitchedEthernetNetDevice::Sleep() entering: " << Simulator::Now() << ", DeviceState: " << m_txMachineState << ", deviceId: " << m_deviceId);
	  if (m_txMachineState == SLEEP)
	  {
		  m_txMachineState = LPI;
		  //Remove NS_LOG_UNCOND(Simulator::Now() << " SwitchedEthernetNetDevice::Sleep() entering LPI State m_deviceId: " << m_deviceId);
		  enter_LPI = Simulator::Now();
		  //REMOVE cout << m_deviceId << " " << enter_LPI << endl;
	  }
	  else
	  {
...
{
	  //Remove 	     NS_LOG_UNCOND("SwitchedEthernetNetDevice::LPIState() entering: " << Simulator::Now() << ", DeviceState: " << m_txMachineState << ", DeviceId: " << m_deviceId);  // Remove
  if (m_queue->IsEmpty() && m_otherEndLink->GetQueue()->IsEmpty())
  {
	  //Remove  	NS_LOG_UNCOND("SwitchedEthernetNetDevice::LPIState() queue is empty, then go to SLEEP again: " << Simulator::Now() << ", DeviceState: " << m_txMachineState << ", DeviceId: " << m_deviceId);   // Remove
	m_txMachineState = SLEEP;
	Simulator::ScheduleNow(&SwitchedEthernetNetDevice::Sleep, this);
  }
  else
  {
	  //Remove    	NS_LOG_UNCOND("SwitchedEthernetNetDevice::LPIState() queue is not empty and so, wake up: " << Simulator::Now() << ", DeviceState: " << m_txMachineState << ", m_deviceId: " << m_deviceId);    // Remove
//	exit_LPI = Simulator::Now();
//	NS_LOG_UNCOND(Simulator::Now() << " SwitchedEthernetNetDevice::LPIState() exiting LPI state, m_deviceId: " << m_deviceId);
//	NS_LOG_UNCOND("Time in LPI: " << exit_LPI-enter_LPI);
...
/* This is the old ExitLPI() used in e-Energy 2013 paper for Dynamic Timer algorithm. Below there is the new one, used for AdaptiveTimer algorithm
  void
SwitchedEthernetNetDevice::ExitLPI (void)
{
  NS_ASSERT(m_txMachineState == LPI);
  m_txMachineState = WAKEUP;
  //Remove NS_LOG_UNCOND("SwitchedEthernetNetDevice::ExitLPI() m_queue timer: " << m_queueTimer << ", m_maxNumberOfPackets: " <<  m_maxNumberOfPackets);
  if (m_dynamicEnable)
  {
    ...
    if (m_queueTimer < m_maxTimeout )
	{
      ...
      else if (m_additiveEnable && m_multiplicativeEnable)
      {
        ...
        if (m_queueTimer > m_maxTimeout)
          m_queueTimer = m_maxTimeout;
        //NS_LOG_UNCOND("new timeout (incremented): " << m_queueTimer);
    ...
    if (m_maxNumberOfPackets >= 2)
    {
      ...
      else if (m_additiveEnable && m_multiplicativeEnable)
      {
        ...
        if (m_maxNumberOfPackets < 2)
		  m_maxNumberOfPackets = 2;
        //NS_LOG_UNCOND("new QSize (decremented): " << m_maxNumberOfPackets);
    ...
    //Remove NS_LOG_UNCOND("SwitchedEthernetNetDevice::ExitLPI()2 m_queue timer: " << m_queueTimer << ", m_maxNumberOfPackets: " <<  m_maxNumberOfPackets);
    Simulator::Schedule(m_wakeUpTime, &SwitchedEthernetNetDevice::WakeUp, this);
...
SwitchedEthernetNetDevice::ExitLPI (void)
{
  ...
  if (m_dynamicEnable)
  {
    int index = int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000);
        //At this point we increase the timer
    if ( ((m_queue->filteredAverageDelay[index])/1000000) < m_maxAverageDelay && ((m_otherEndLink->m_queue->filteredAverageDelay[index])/1000000) < m_maxAverageDelay)
    {
      if (m_queueTimer < m_maxTimeout )
      {
        if (m_additiveEnable && !m_multiplicativeEnable)
        {
          ...
          if (m_queueTimer > m_maxTimeout)
            m_queueTimer = m_maxTimeout;
//          cout << "new timeout (incremented1): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay/1000000 << " Time: " << Simulator::Now() << " Delay: " << m_queue->partialDelay[(Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000] << " Packets: " << partialPackets[(Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000] << endl;
          cout << "new timeout (incremented1): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;
        }
        else if (!m_additiveEnable && m_multiplicativeEnable)
        {
          double gama = 1+(double(m_timerStepMultiplicative)/100);
          ...
          if (m_queueTimer > m_maxTimeout)
            m_queueTimer = m_maxTimeout;
          cout << "new timeout (incremented2): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<   " CyclePowerSave: " << cycle_LPI/totalCycle << endl;
        }
          //in this case we have additive increase multiplicative decrease
        else if (m_additiveEnable && m_multiplicativeEnable)
        {
          ...
          if (m_queueTimer > m_maxTimeout)
            m_queueTimer = m_maxTimeout;
          //NS_LOG_UNCOND("new timeout (incremented): " << m_queueTimer);
          cout << "new timeout (incremented3): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;
        }

        else
          cout << "SwitchedEthernetNetDevice::ExitLPI() ERROR: both variables are false!";
      }
      else if (m_queueTimer == m_maxTimeout) 
	cout << "new timeout (constant): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() <<  " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;
    }
    else if ( ((m_queue->filteredAverageDelay[index])/1000000) >= m_maxAverageDelay)
    {
      if (m_queueTimer > MicroSeconds(100.0) )
      {
        if (m_additiveEnable && !m_multiplicativeEnable)
        {
          double alpha = double(m_timerStepAdditive)/1000000;
          if (alpha >= m_queueTimer)
            m_queueTimer = MicroSeconds(100.0);
          else
            m_queueTimer = ns3::Time::FromDouble(m_queueTimer.ToDouble(ns3::Time::Unit(0))-alpha, ns3::Time::Unit(0));
          if (m_queueTimer < MicroSeconds(100.0))
            m_queueTimer = MicroSeconds (100.0);
          cout << "new timeout (decremented4): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;

//          cout << "new timeout (decremented4): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << endl;
        }
        else if (!m_additiveEnable && m_multiplicativeEnable)
        {
          ...
          if (m_queueTimer < MicroSeconds(100.0))
            m_queueTimer = MicroSeconds (100.0);
          cout << "new timeout (decremented5): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << cycle_LPI/totalCycle <<  endl;
        }
        else if (m_additiveEnable && m_multiplicativeEnable)
        {
          ...
          if (m_queueTimer < MicroSeconds(100.0))
            m_queueTimer = MicroSeconds (100.0);
        //NS_LOG_UNCOND( "new timeout (decremented): " << m_queueTimer);
          cout << "new timeout (decremented6): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;
        }
        else
          cout << "SwitchedEthernetNetDevice::ExitLPI() ERROR: both variables are false!";
      }
      else if (m_queueTimer == MicroSeconds(100.0) )
        cout << "new timeout (constant2): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() <<  " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;
    }
    else
      cout << "new timeout (constant3): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() <<  " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 << " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" <<  " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;
...
{
//	  NS_LOG_UNCOND("SwitchedEthernetNetDevice::WakeUp() complete: " << Simulator::Now() << ", device: " << m_deviceId);
  m_txMachineState = READY;
  exit_LPI = Simulator::Now();
  cycle_LPI = exit_LPI - enter_LPI - m_wakeUpTime;
  //REMOVE cout << m_deviceId << " " << exit_LPI- m_wakeUpTime << " " << exit_LPI - enter_LPI - m_wakeUpTime << endl;
  total_LPI = total_LPI + (exit_LPI - enter_LPI - m_wakeUpTime);

  endCycle = Simulator::Now();
  totalCycle = endCycle-startCycle;

  startCycle = Simulator::Now();

  int index = int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)+1;
  hundredms_LPI[index] = hundredms_LPI[index] + cycle_LPI.GetNanoSeconds();
  hundredms_cycle[index] = hundredms_cycle[index] + totalCycle.GetNanoSeconds();
  //Remove NS_LOG_UNCOND(Simulator::Now() << " SwitchedEthernetNetDevice::WakeUp() exiting LPI state, m_deviceId: " << m_deviceId);
  //Remove NS_LOG_UNCOND("Time in LPI: " << exit_LPI-enter_LPI-m_wakeUpTime);
  //Remove NS_LOG_UNCOND("Total time in LPI: " << total_LPI);
...
{
//  NS_LOG_UNCOND("SwitchedEthernetNetDevice::ChannelIsActive(): State 0 : " << m_channel->GetState(0) << ", State 1 : " << m_channel->GetState(1));
...
{
	  //Remove 	   	   NS_LOG_UNCOND("SwitchedEthernetNetDevice::EnableSleep() Device: " << m_deviceId << ", state: " << m_txMachineState);   // Remove
	  if(m_txMachineState == SLEEP) return; //already sleeping
//	  NS_LOG_UNCOND("SwitchedEthernetNetDevice::EnableSleep() Device: " << m_deviceId << ", goes to sleep if it has nothing to transmit (transition from Active->LPI). " << Simulator::Now());
	  m_txMachineState = SLEEP;
...
{
	//Remove   NS_LOG_UNCOND("SwitchedEthernetNetDevice::CancelSleep() Cancelling sleep state in case a packet arrives during transition " << Simulator::Now());   // Remove
  m_txMachineState = READY;
  Simulator::Cancel(id_sleep);
}


/*The old CancelLPI() is commented below*/

  void
SwitchedEthernetNetDevice::CancelLPI (void)
{
	  //Remove NS_LOG_UNCOND("SwitchedEthernetNetDevice::CancelLPI(), device:" << m_deviceId << ". Cancel the LPI timer since channel was activated before the timer expired. " << Simulator::Now());  // Remove
  ...
  //Remove NS_LOG_UNCOND("SwitchedEthernetNetDevice::CancelLPI() m_queue timer: " << m_queueTimer << ", m_maxNumberOfPackets: " <<  m_maxNumberOfPackets);
  if (m_dynamicEnable)
  {
    int index = int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000);
    ...
    if ( ((m_queue->filteredAverageDelay[index])/1000000) < m_maxAverageDelay && ((m_otherEndLink->m_queue->filteredAverageDelay[index])/1000000) < m_maxAverageDelay)
    {
      if (m_queueTimer < m_maxTimeout )
      {
        ...
        {
          //double beta = double(m_timerStepAdditive)/1000000;
          //m_queueTimer = ns3::Time::FromDouble(m_queueTimer.ToDouble(ns3::Time::Unit(0))+beta, ns3::Time::Unit(0));   //increase of the timer
          if (m_queueTimer > m_maxTimeout)
            m_queueTimer = m_maxTimeout;
          cout << "new timeout (incremented1): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << " Delay[" << int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000) << "]: " << double(m_queue->partialDelay[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)]) << " Packets: " << m_queue->partialPackets[int((Simulator::Now()-m_queue->firstPacketTime).GetNanoSeconds()/100000000)] <<  " AverageDelay[100ms]: " << (m_queue->filteredAverageDelay[index])/1000000 << " GlobalAverageDelay[ms]: " << m_queue->globalAverageDelay/1000000 <<  " PowerSave: "<< total_LPI.GetSeconds()/(Simulator::Now()-simulation_start).GetSeconds()*100 << " %" << " CyclePowerSave: " << (cycle_LPI.GetSeconds()/totalCycle.GetSeconds())*100 << " % PowerSave[100ms]: " << hundredms_LPI[index]*100/hundredms_cycle[index] << " % DelayDeviation: " << m_queue->delayDeviation[index]*1000 << endl;
//          cout << "new timeout (incremented1): " << m_queueTimer << " deviceID: " << m_deviceId << " Delay: " << m_queue->filteredAverageDelay[index]/1000000 << " Time: " << Simulator::Now() << endl;
        }
        else if (!m_additiveEnable && m_multiplicativeEnable)
        {
          double gama = 1+(double(m_timerStepMultiplicative)/100);
...
//    m_queueTimer = ns3::Time::FromDouble(m_queueTimer.ToDouble(ns3::Time::Unit(0))-alpha, ns3::Time::Unit(0));
  ...
  Simulator::Schedule(m_wakeUpTime, &SwitchedEthernetNetDevice::WakeUp, this);
  //Remove NS_LOG_UNCOND("SwitchedEthernetNetDevice::CancelLPI()2 m_queue timer: " << m_queueTimer << ", m_maxNumberOfPackets: " <<  m_maxNumberOfPackets);
}
/*
  void
SwitchedEthernetNetDevice::CancelLPI (void)
...
SwitchedEthernetNetDevice::SendFrom (Ptr<Packet> packet, const Address& src, const Address& dest, uint16_t protocolNumber)
{
  ...
  if (simulation_start == Seconds (0))
  {
    ...
    m_otherEndLink->simulation_start = simulation_start;
    NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom() started: " << simulation_start);
  }
  //
  ...
  //Remove NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom(), a packet just arrived: " << m_deviceId << ", at" << Simulator::Now());  // Remove
...
//	{
	  //Remove  	  NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom() cancelling id_sleep, device: " << m_deviceId << ", at" << Simulator::Now());   // Remove
	  CancelSleep();
...
//	}
  ...
  if (m_txMachineState == READY) 
  {
	  ...
	  m_promiscSnifferTrace (m_currentPkt);
	  m_snifferTrace (m_currentPkt);
	  //Remove  	  NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom(), initiate a tx, device: " << m_deviceId << ", at: " << Simulator::Now());   // Remove
	  TransmitStart ();
  }
  ...
  {
//	if (m_queue->GetNPackets() >= m_maxNumberOfPackets || ChannelIsActive())
//	{
	  //Remove NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom(), arrival in LPI state, device: " << m_deviceId << ", at: " << Simulator::Now());  // Remove
		if( m_queue->GetNPackets() >= m_maxNumberOfPackets )
		{
			//Remove  		    NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom(), oops, the coalescing buffer is full!!! device: " << m_deviceId << ", at: " << Simulator::Now());   // Remove
			CancelLPI();
			m_otherEndLink->CancelLPI();
		}
/*	  	if (ChannelIsActive())
	  	{
		  NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom, Machine State equals to LPI and channel is active for device: " << m_deviceId << ", at: " << Simulator::Now());
		  Simulator::ScheduleNow(&SwitchedEthernetNetDevice::WakeUp, this);
	  	}
	  	else if (ChannelIsActive() == false)
	  	{
			NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom, Machine State equals to LPI and channel is not active for device: " << m_deviceId);
			Simulator::ScheduleNow(&SwitchedEthernetNetDevice::LPIState, this);
	  	}
*/
		//if (m_queue->GetNPackets() >= m_maxNumberOfPackets)
			//{
			//if (m_queueTimer >= MilliSeconds(2.5))
				//m_queueTimer = m_queueTimer - MicroSeconds(400);
//	    if (m_maxNumberOfPackets <= 200)
//	      m_maxNumberOfPackets = m_maxNumberOfPackets*1.1;
//	    NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom()1 new Timer: " << m_queueTimer << ", new MaxPackets: " << m_maxNumberOfPackets << ", m_deviceId: " << m_deviceId);
//	}
		else if( m_queue->GetNPackets() == 1 && m_otherEndLink->GetQueue()->IsEmpty())
		{
			if (m_queueTimer > m_otherEndLink->m_queueTimer)
			  m_queueTimer = m_otherEndLink->m_queueTimer;
			else
			  m_otherEndLink->m_queueTimer = m_queueTimer;
			//Remove 			NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom(), only 1 packet in the buffer, 0 in the other: time to start a timer. Device: " << m_deviceId << ", at: " << Simulator::Now());  // Remove
		...
		//	else if (m_queue->GetNPackets() < m_maxNumberOfPackets)
		//	{
		//		if (m_queueTimer <= MilliSeconds(11.5) && ChannelIsActive() == false)
		//			m_queueTimer = m_queueTimer + MicroSeconds(400);
//		if (m_maxNumberOfPackets >= 50 && ChannelIsActive() == false)
//		  m_maxNumberOfPackets = m_maxNumberOfPackets*0.9;
//		NS_LOG_UNCOND("SwitchedEthernetNetDevice::SendFrom()2 new Timer: " << m_queueTimer << ", new MaxPackets: " << m_maxNumberOfPackets << ", m_deviceId: " << m_deviceId);
			//}
//	}
  }
  //Added by Angelos END
