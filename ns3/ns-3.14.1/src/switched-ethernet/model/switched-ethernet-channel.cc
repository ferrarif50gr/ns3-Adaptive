...
void
SwitchedEthernetChannel::Print(uint32_t deviceId)
{
	endpoints[deviceId]->PrintMessage();
/*  NS_LOG_UNCOND("SwitchedEthernetChannel::PrintMessage(void) endpoints[deviceId]: " \
		  << endpoints[deviceId] << ", endpoints[other]: "*/ /*
		  << endpoints[deviceId]->SetOtherLinkEnd(endpoints[deviceId])*/ /*);*/
}
